...
class Report{
  ...
  public function all_queues_index()
  {

    $queues = DB::getInstance()->select('distinct queue','datamart_queue_details','');
    $_SESSION['queues']=$queues;
    return header('location:../view/all_queues_interval/index.php');
  }
    public function login_index()
  {
    ...
    $groups = DB::getInstance()->select('*','cfg_agent_group','');
    $_SESSION['agent_group']=$groups;
    return header('location:../view/login/index.php');
  }
    public function not_ready_index()
  {

    $groups = DB::getInstance()->select('*','cfg_agent_group','');
    $_SESSION['agent_group']=$groups;
    return header('location:../view/not_ready/index.php');
  }
...
public function get_all_queue_interval_report($sdate,$edate,$start_int_date,$end_int_date,$queues = array())
  {
   $queue = implode('","',$queues);
   $data =DB::getInstance()->select('*','datamart_queue_details','queue in("'.$queue.'") and event_time>="'.$sdate.'" and event_time<="'.$edate.'" and TRACKNUM in (select TRACKNUM from datamart_queue_details where EVENT_TIME >= "'.$start_int_date.'" and event_time <= "'.$end_int_date.'" and event = "queued") order by tracknum , event_time ');
...
//return header('locaction:/')
  ...
  public function get_agent_details($sdate,$edate,$agent_id)
  {
      ...
      if ( $data) {
        ...
        foreach ($data as $key => $row) {
          ...
          else if ($row['EVENT']=='LOGIN' && $logout_flag==1) {
            $current_event_time = $row['EVENT_TIME'];
            $logout_login_differance+= strtotime($current_event_time) - strtotime($logout_event);
            $logout_flag=0;
          }
          ...
          $shift_time = (strtotime($last_event)-strtotime($first_event))-$logout_login_differance;
  ...
  public function get_agent_login_report($sdate,$edate,$group)
  {
      $agents = DB::getInstance()->select('EVENT_TIME,AGENT_ID,EVENT,FIRST_NAME,LAST_NAME','datamart_agent_details left join cfg_person on agent_id=login_id','grp_dbid="'.$group.'" and  event in ("LOGIN","LOGOUT") and event_time>="'.$sdate.'" and event_time<="'.$edate.'" order by agent_id,event_time');
      if ($agents) {
        $_SESSION['get_login_report_data'] = $agents;
         return header('location:../view/login/view_data.php');
      }    
  }
  public function get_not_ready_report($sdate,$edate,$group)
  {
        $agents = DB::getInstance()->select('EVENT_TIME,AGENT_ID,EVENT,FIRST_NAME,LAST_NAME,EVENTDETAILS','datamart_agent_details left join cfg_person on agent_id=login_id','grp_dbid="'.$group.'" and   EVENTDETAILS <> "" and  event="NOT_READY" and event_time>="'.$sdate.'" and event_time<="'.$edate.'" order by agent_id,event_time');
        if ($agents) {
          $_SESSION['get_notready_report_data'] = $agents;
           return header('location:../view/not_ready/view_data.php');
        }  
  }  
...
else if (isset($_POST['all_queue_inteval_submit'])) {
    require_once '../core/init.php';
if (!empty($_POST['Sdate']) && !empty($_POST['Edate']) && !empty($_POST['queue'])) {
             $report_iterval_data=array();
       $interval   = $_POST["interval"];
       ...
       $interval_queue = new Report();  
        while (strtotime($end_int_date)<=strtotime($_POST['Edate'])){
         ...
         $report_iterval_data [] = $interval_queue->get_all_queue_interval_report($_POST['Sdate'],$_POST['Edate'],$start_int_date,$end_int_date,$queue);
         $start_int_date = $end_int_date;
         $end_int_date = strtotime($start_int_date) + $interval;
         $end_int_date = date('Y-m-d H:i:s', $end_int_date);             
        } 

                $_SESSION['get_all_queue_interval_report_data'] = $report_iterval_data;  
          return header('location:../view/all_queues_interval/view_data.php');                  
}
}
...
else if(isset($_POST['agent_login_submit'])){
  require_once '../core/init.php';
  if (!empty($_POST['Sdate']) && !empty($_POST['Edate']) && !empty($_POST['group'])) {
    $report = new Report();
    $report_data=array();
    foreach ($_POST['group'] as  $group) {
          $report_data []= $report->get_agent_login_report($_POST['Sdate'],$_POST['Edate'],$group);
        }   
  }  
}
elseif (isset($_POST['not_ready_submit'])) {
  require_once '../core/init.php';
  if (!empty($_POST['Sdate']) && !empty($_POST['Edate']) && !empty($_POST['group'])) {
    $report = new Report();
    $report_data=array();
    foreach ($_POST['group'] as  $group) {
          $report_data []= $report->get_not_ready_report($_POST['Sdate'],$_POST['Edate'],$group);
        }   
  }
}
...
}else if (isset($_POST['all_queues_inteval_submit']))
{
  echo "here";
}
...
if (isset($_GET['report'])) {
  ...
  elseif ($_GET['report']=='all_queues_report') {
    $view_report = new Report();
    $view_report->all_queues_index(); 
  }
  elseif ($_GET['report']=='login_report') {
    $view_report = new Report();
    $view_report->login_index(); 
  }
  elseif ($_GET['report']=='not_ready_report') {
    $view_report = new Report();
    $view_report->not_ready_index(); 
  }
